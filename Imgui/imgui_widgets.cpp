...
bool SliderBehaviorAAAAA(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
    ...
    // Draw frame
    ImGui::RenderFrame(frame_bb.Min, frame_bb.Max, ImGui::ColorConvertFloat4ToU32(ImColor(ImVec4(0.06f, 0.06f, 0.06f, 1.0f))), false, style.FrameRounding);
    ...
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), ImVec2(frame_bb.Max.x, frame_bb.Min.y + 10), ImColor(0, 0, 0), 2);
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 5, grab_bb.Min.y + 8), 4, ImColor(255, 0, 0), 128);
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), ImVec2(grab_bb.Min.x, frame_bb.Min.y + 10), ImColor(255, 0, 0), 2);
